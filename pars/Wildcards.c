...
int	wildcard_match(const char *pattern, const char *str)
{
	...
	int backtrackSIndex = -1;
		// Corresponding position in the string to backtrack to
	...
	while (str[sIndex] != '\0') // Iterate through the string
	{
		...
		else if (pattern[pIndex] == str[sIndex])
			// Direct match or single character match *
		{
			pIndex++; // Move to the next pattern character
			sIndex++; // Move to the next string character
		}
		else if (backtrackPIndex != -1)
			// Backtrack if there was a '*' previously
		{
			pIndex = backtrackPIndex + 1;
				// Reset to the position after the last '*'
			sIndex = ++backtrackSIndex;  
...
void	replace_wildcard(char ***args, int *i, MemoryManager *manager)
{
	...
	while ((entry = readdir(dir)) != NULL)
	{
		...
		{
			match_count++;
			matches = gc_realloc(manager, matches, (match_count)
					* sizeof(char *));
	...
	old_size = ft_strlen1(*args);
	new_size = old_size + match_count - 1;
